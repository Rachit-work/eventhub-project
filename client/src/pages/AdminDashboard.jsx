...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        const res = await fetch('https://eventhub-project-w814.onrender.com/api/admin/overview');
        if (res.ok) {
  ...
  const handleApprove = async (firebase_uid) => {
    try {
      const res = await fetch('https://eventhub-project-w814.onrender.com/api/admin/approve-vendor', {
        method: 'POST',
